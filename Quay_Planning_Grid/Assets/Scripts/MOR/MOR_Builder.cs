...
public class MOR_Builder : MonoBehaviour
{
    public static MOR_Builder Instance { get; private set; }

    public GameObject shipPrefab; // assign in inspector
    private MOR_Data simData;
    ...
    public void InitializeBuilder()
    {
        if (simData == null)
        {
            simData = FindObjectOfType<MOR_Data>();
        }

        if (simData == null)
        {
            Debug.LogError("MOR_Builder: No MOR_Data found in the scene!");
            return;
        }
        simData.LoadSimulationLogs("log-MOR-MF.csv");
    ...
    void SpawnShip(ShipData shipData)
    {
        ...
        GameObject sourceWP = MOR_WPManager.Instance.GetWaypoint("Source");
        if (sourceWP == null)
        {
        ...
        MOR_Runtime runtime = newShip.GetComponent<MOR_Runtime>();
        if (runtime == null)
        {
            runtime = newShip.AddComponent<MOR_Runtime>();
        }
        ...
        if (simData != null)
        {
            ...
            Debug.Log("SimulationData.Instance = " + MOR_Data.Instance);
